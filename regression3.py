...
# Set an initial threshold for RMSE
rmse_threshold = 1.0
...
while True:
    ...
    # Print the Mean Squared Error with simplified format
    print(f'Mean Squared Error:',mse)

    # Print the Root Mean Squared Error with simplified format
    print(f'Root Mean Squared Error:',rmse)
    print(f'R-squared: {r_squared:.6f}')

    if rmse <= rmse_threshold:
        print(f'RMSE is less than or equal to the threshold. Stopping iterations.')
        break

# Update only the 'PM2.5' column in the dataframe for the next iteration
    df_imputed.loc[X_test.index, 'PM2.5'] = y_pred
